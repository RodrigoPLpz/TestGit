...
str(baby)
# El total de bebes nacidos para los estados de mi poblaci�n de estudio es de 713384
# M�xico ocupa el primer lugar en embarazo adolescente a nivel mundial. 
# �Cu�l es el promedio de edad de una mujer que da a luz?
# �Qu� estados tienen m�s mam�s adolescentes?
...
gr <- ggplot(tempo,aes(x = entidad_nacimiento, y = total_nac)) + geom_point(color="blue", size=5.5) +
      ...
      labs(title = "N�mero de nacimientos por Estado", subtitle = "2016", x="Estados", y="Total de nacidos") +
      theme(axis.text.x = element_blank())

ggsave(paste(dir2,"1.png",sep = "/"),plot = gr, width = 10, height = 10)      

# Cantidad de nacidos por entidad federativa y el promedio de edad de las mam�s.
...
gr <- ggplot(tempo,aes(x = entidad_nacimiento, y = total_nac)) + geom_bar(stat = "identity", fill = "#2c7fb8") +
      ...
      labs(title = "N�mero de nacimientos y promedio de edad de las madres  por Estado", subtitle = "2016", x = "Estado", y = "Total de nacidos")
...
# Madres adolescentes de 12 - 20 a�os por entidad federativa.
...
treemap(tempo, index ="entidad_nacimiento", vSize = "porcentaje", vColor = "index", type = "index",
        title = "Porcentaje de madres adolecentes de 12 a 20 a�os por Estado - 2016", palette = "RdYlBu",
        title.legend = "", border.col = "grey", border.lwd=0.5)
dev.off()

#svg(paste(dir2, "6.svg", sep = "/"), width = 12, height = 12)
#treemap(tempo, index ="entidad_nacimiento", vSize = "porcentaje", vColor = "index", type = "index",
        #title = "Porcentaje de m�mas adolecentes por estado - 2016", palette = "RdYlBu",
        #title.legend = "", border.col = "grey", border.lwd=0.5)
#dev.off()

# Madres adolescentes de 12 - 20 a�os por entidad federativa. Descartando los casos de ni�as embarazadas.
tempo <- baby%>%
         select(id, edad_madre, rango_edad,entidad_nacimiento) %>%
         filter(edad_madre > 11 & edad_madre < 21 ) %>%
         group_by(entidad_nacimiento,rango_edad) %>%
...
gr <- ggplot(tempo, aes(x = rango_edad, y = porcentaje)) +
      ...
      labs(title = "Porcentaje de madres adolecentes de 12 a 20 a�os por Estado - 2016", x = "Rango de edad", 
           y = "Porcentaje") +
...
gr <- ggplot(tempo, aes(x = rango_edad, y = total_nac, fill = rango_edad)) + geom_boxplot() +
      labs(title = "Madres adolescentes de 12 - 20 a�os por cantidad de bebes nacidos y rango de edad",
           x = "Rango de edad", y = "Total de nacidos", fill="") + theme_bw() + 
      theme(axis.text.x = element_blank(),legend.position = "bottom")

ggsave(paste(dir2,"7.png",sep = "/"), plot = gr, width = 11, height = 10)

summary(tempo$total_nac[tempo$rango_edad == "12 - 18 años"])

summary(tempo$total_nac[tempo$rango_edad == "19 - 25 años"])
